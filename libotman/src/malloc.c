#include "libotman.h"

struct chunk {
  long sig;
  unsigned int prev;
  unsigned int size;
};

struct meta {
  void *fd;
  void *bk;
};

void *base = 0;
void *top = 0;

struct meta *freed = 0;
long key = 0xdeadbeef;

void *brk(void *brk) {
  void *ret;
  __asm__ volatile("mov $12, %%rax\n"
                   "mov %1, %%rdi\n"
                   "syscall\n"
                   : "=a"(ret)
                   : "g"(brk)
                   : "rdi", "memory");

  return ret;
}

#define FREE 0x1

#define ptr2chk(ptr) (struct chunk *)((long)ptr - sizeof(struct chunk))
#define chk2ptr(ptr) (void *)((long)ptr + sizeof(struct chunk))
#define chk2meta(ptr) (struct meta *)((long)ptr + sizeof(struct chunk))
#define getsize(chk) (chk->size & ~0xf)
#define align(size) ((size + 0xf) & ~0xf)
#define isfree(chk) (chk->size & FREE)
#define inheap(chk) ((void *)chk >= base && (void *)chk < top)
#define checksig(chk) (chk->sig == sig(chk))

long sig(struct chunk *chk) {
  unsigned long cookie = chk->size + chk->prev + key;
  if (isfree(chk)) {
    struct meta *meta = chk2meta(chk);
    cookie += (long)meta->fd + (long)meta->bk;
  };
  return crc32((unsigned char *)&cookie, sizeof(unsigned long));
}

void *malloc(size_t size) {
  void *ret = (void *)-1;

  if (size > UINT_MAX - 16)
    return ret;

  if (!base) {
    top = base = brk(0);
    getrandom((char *)&key, 8);
  }

  size = align(size);

  if (!freed) {
    struct chunk *chk = (struct chunk *)top;
    top = brk(chk2ptr((char *)top + size));
    chk->size = size;
    chk->sig = sig(chk);
    chk->prev = 0;
    ret = chk2ptr(chk);
    goto ret;
  }

  struct meta *curr = freed;

  while (curr && inheap(curr)) {
    struct chunk *chk = ptr2chk(curr);
    size_t chk_size = getsize(chk);

    if (!checksig(chk)) {
      puts("invalid malloc operation (invalid sig)\n");
      exit(-1);
    }

    if (chk_size < size) {
      curr = curr->fd;
      continue;
    }

    struct meta *next_meta = curr->fd;
    struct meta *prev_meta = curr->bk;
    void *updated_bk = prev_meta;
    void *updated_fd = next_meta;

    if (chk_size >= size + sizeof(struct chunk) + sizeof(struct meta)) {
      struct chunk *new_chk =
          (struct chunk *)((char *)chk + size + sizeof(struct chunk));
      struct meta *new_meta = chk2meta(new_chk);

      new_chk->prev = size;
      new_chk->size = (chk_size - size - sizeof(struct chunk)) | FREE;

      new_meta->fd = next_meta;
      new_meta->bk = prev_meta;

      struct chunk *next_chk =
          (struct chunk *)((char *)new_chk + getsize(new_chk) +
                           sizeof(struct chunk));
      if (inheap(next_chk)) {
        next_chk->prev = getsize(new_chk);
        next_chk->sig = sig(next_chk);
      }

      updated_fd = updated_bk = new_meta;
      new_chk->sig = sig(new_chk);
      chk->size = size;
    } else {
      chk->size = chk_size;
    }

    if (next_meta) {
      next_meta->bk = updated_bk;
      struct chunk *next_chk = ptr2chk(next_meta);
      next_chk->sig = sig(next_chk);
    }

    if (prev_meta) {
      prev_meta->fd = updated_fd;
      struct chunk *prev_chk = ptr2chk(prev_meta);
      prev_chk->sig = sig(prev_chk);
    } else
      freed = (struct meta *)updated_fd;

    curr->fd = 0;
    curr->bk = 0;
    chk->sig = sig(chk);

    ret = chk2ptr(chk);
    goto ret;
  }

  struct chunk *chk = (struct chunk *)top;
  top = brk(chk2ptr((char *)top + size));

  chk->size = size;
  chk->prev = 0;
  chk->sig = sig(chk);
  ret = chk2ptr(chk);
ret:
  return ret;
}

void free(void *ptr) {
  if (!ptr)
    return;

  if (!inheap(ptr)) {
    puts("invalid free operation (out of bounds)\n");
    exit(-1);
  }

  struct chunk *chk = ptr2chk(ptr);

  if (isfree(chk)) {
    puts("invalid free operation (double free)\n");
    exit(-1);
  }

  if (!checksig(chk)) {
    puts("invalid free operation (invalid sig)\n");
    exit(-1);
  }

  size_t chk_size = getsize(chk);
  chk->size = chk_size;
  struct meta *meta = (struct meta *)ptr;

  struct chunk *next_chk = (struct chunk *)((char *)ptr + chk_size);
  if (inheap(next_chk) && isfree(next_chk) && checksig(next_chk)) {
    size_t next_size = getsize(next_chk);
    chk_size += next_size + sizeof(struct chunk);

    struct meta *next_meta = chk2meta(next_chk);
    struct meta *n_fd = next_meta->fd;
    struct meta *n_bk = next_meta->bk;

    if (n_fd) {
      n_fd->bk = n_bk;
      struct chunk *c = ptr2chk(n_fd);
      c->sig = sig(c);
    }
    if (n_bk) {
      n_bk->fd = n_fd;
      struct chunk *c = ptr2chk(n_bk);
      c->sig = sig(c);
    } else {
      freed = n_fd;
    }

    next_chk->size = 0;
    next_chk->prev = 0;
    next_chk->sig = 0;
    next_meta->fd = 0;
    next_meta->bk = 0;
  }

  struct chunk *prev_chk =
      (struct chunk *)((char *)chk - chk->prev - sizeof(struct chunk));

  if (prev_chk && inheap(prev_chk) && isfree(prev_chk) && checksig(prev_chk)) {
    size_t prev_size = getsize(prev_chk);
    chk_size += prev_size + sizeof(struct chunk);

    chk->size = 0;
    chk->prev = 0;
    chk->sig = 0;
    meta->fd = 0;
    meta->bk = 0;

    chk = prev_chk;
  } else {
    meta->fd = freed;
    meta->bk = 0;

    if (freed) {
      freed->bk = meta;
      struct chunk *f_chk = ptr2chk(freed);
      f_chk->sig = sig(f_chk);
    }
    freed = meta;
  }

  struct chunk *following_chk =
      (struct chunk *)((char *)chk + chk_size + sizeof(struct chunk));
  if (inheap(following_chk)) {
    following_chk->prev = chk_size;
    following_chk->sig = sig(following_chk);
  }

  chk->size = chk_size | FREE;
  chk->sig = sig(chk);
}

void *realloc(void *ptr, size_t size) {
  if (size > UINT_MAX - 16)
    return (void *)-1;

  if (!ptr)
    return malloc(size);

  if (!size){
    free(ptr);
    return 0;
  }

  if (!inheap(ptr)) {
    puts("invalid realloc operation (out of bounds)\n");
    exit(-1);
  }

  struct chunk *chk = ptr2chk(ptr);

  if (isfree(chk)) {
    puts("invalid realloc operation (chunk is free)\n");
    exit(-1);
  }

  if (!checksig(chk)) {
    puts("invalid realloc operation (invalid sig)\n");
    exit(-1);
  }

  void *ret = ptr;
  size = align(size);

  size_t chk_size = getsize(chk);
  struct chunk *following_chk =
      (struct chunk *)((char *)chk + chk_size + sizeof(struct chunk));

  if (size == chk_size)
    goto ret;

  if (size > chk_size) {
    unsigned int more = size - chk_size;

    if (inheap(following_chk) && isfree(following_chk) &&
        getsize(following_chk) > more) {
      chk->size = size;
      chk->sig = sig(chk);

      struct chunk *new_chk = (struct chunk *)((char *)ptr + size);
      struct meta *new_meta = chk2meta(new_chk);
      struct meta *following_meta = chk2meta(following_chk);
      int new_size = getsize(following_chk) - more;
      struct meta *new_fd = following_meta->fd;
      struct meta *new_bk = following_meta->bk;

      struct chunk *next_chunk =
          (struct chunk *)((char *)following_chk + getsize(following_chk) +
                           sizeof(struct chunk));
      if (inheap(next_chunk)) {
        next_chunk->prev = new_size;
        next_chunk->sig = sig(next_chunk);
      }

      following_chk->prev = 0;
      following_chk->size = 0;
      following_chk->sig = 0;
      following_meta->fd = 0;
      following_meta->bk = 0;

      new_chk->size = new_size | FREE;
      new_chk->prev = size;
      new_meta->fd = new_fd;
      new_meta->bk = new_bk;
      new_chk->sig = sig(new_chk);

      if (new_fd) {
        new_fd->bk = new_meta;
        struct chunk *c = ptr2chk(new_fd);
        c->sig = sig(c);
      }
      if (new_bk) {
        new_bk->fd = new_meta;
        struct chunk *c = ptr2chk(new_bk);
        c->sig = sig(c);
      } else {
        freed = new_meta;
      }
      goto ret;
    }

    ret = malloc(size);
    memcpy(ret, ptr, chk_size);
    free(ptr);
    goto ret;
  } else {
    int less = chk_size - size - sizeof(struct chunk);
    if (less < (int)sizeof(struct meta)) {
      ret = malloc(size);
      memcpy(ret, ptr, size);
      free(ptr);
      goto ret;
    }
    chk->size = size;
    chk->sig = sig(chk);

    struct chunk *new_chk = (struct chunk *)((char *)ptr + size);
    struct meta *new_meta = chk2meta(new_chk);
    new_chk->size = (less) | FREE;
    new_chk->prev = size;
    new_meta->fd = freed;
    new_meta->bk = 0;

    if (freed) {
      freed->bk = new_meta;
      struct chunk *c = ptr2chk(freed);
      c->sig = sig(c);
    }

    freed = chk2meta(new_chk);
    new_chk->sig = sig(new_chk);

    if (!inheap(following_chk))
      goto ret;

    following_chk->prev = less;
    following_chk->sig = sig(following_chk);
  }

ret:
  return ret;
}
